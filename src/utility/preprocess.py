...
# =============================================================================
print("\n[1/6] Processing Precipitation (CHIRPS) ...")
(PROCESSED / "precip_chirps_daily.csv").unlink(missing_ok=True)

precip_daily = pd.read_csv(CHIRPS_FILE, parse_dates=["date"])
precip_daily = clip_to_daterange(precip_daily)

neg_mask = precip_daily["precip_mm_day"] < 0
if neg_mask.sum() > 0:
    print(f"   WARNING: {neg_mask.sum()} negative values set to 0")
    precip_daily.loc[neg_mask, "precip_mm_day"] = 0.0

outliers = flag_outliers(precip_daily["precip_mm_day"])
print(f"   Outlier check (IQRx3): {outliers.sum()} extreme values flagged")

precip_daily.to_csv(PROCESSED / "precip_chirps_daily.csv", index=False)
print(f"   Saved → precip_chirps_daily.csv  ({len(precip_daily)} rows)")
print(f"   Mean: {precip_daily['precip_mm_day'].mean():.2f} mm/day | "
      f"Max: {precip_daily['precip_mm_day'].max():.2f} mm/day")
...
precip_df = pd.read_csv(PROCESSED / "precip_chirps_daily.csv", parse_dates=["date"])
