...
print("Nahr Ibrahim — CNN-LSTM")
print(
    f"  CNN {CFG['cnn_filters']} k={CFG['kernel_size']}  "
    f"LSTM {CFG['lstm_units']}  lr={CFG['lr']}\n"
)
...
# ── model ──────────────────────────────────────────────────────────────────────
def build_cnn_lstm(cfg):
    inp = tf.keras.Input(shape=(cfg["lookback"], cfg["n_features"]), name="input")
    x = inp

    # CNN: pick up local multi-day patterns across all features
    for filters in cfg["cnn_filters"]:
        ...
        x = tf.keras.layers.Dropout(cfg["cnn_dropout"])(x)

    # commented out — MaxPooling made things worse on this dataset
    # x = tf.keras.layers.MaxPooling1D(pool_size=2)(x)

    # LSTM: learn how those patterns evolve over the window
    for i, units in enumerate(cfg["lstm_units"]):
        ...
        x = tf.keras.layers.Dropout(cfg["lstm_dropout"])(x)
    ...
    return tf.keras.Model(inp, out, name="CNN_LSTM_NahrIbrahim")
...
# ── callbacks ──────────────────────────────────────────────────────────────────
ckpt_path = str(MODEL_DIR / "checkpoints" / "cnn_lstm_best.keras")
log_path = str(MODEL_DIR / "configs" / "cnn_lstm_training_log.csv")
...
pd.DataFrame([CFG]).to_csv(MODEL_DIR / "configs" / "cnn_lstm_config.csv", index=False)
...
df_met.to_csv(MET_DIR / "cnn_lstm_metrics.csv", index=False)
...
print("-" * 50)
for m in ["NSE", "KGE", "RMSE", "Peak_Bias_%"]:
    print(f"{m:<14} {lstm[m]:>10} {transf[m]:>12} {m_test[m]:>10}")
...
for label, dates, yt, yp in [
    ...
    ).to_csv(PRED_DIR / f"cnn_lstm_predictions_{label}.csv", index=False)

# ── plots ──────────────────────────────────────────────────────────────────────
log = pd.read_csv(log_path)
best_epoch = log["val_loss"].idxmin()
BG, PANEL = "#080f1a", "#0d1825"
TEAL, BLUE, ORG, RED = "#00b4a0", "#3b9eff", "#f4a261", "#e76f51"
...
ax.axvline(best_epoch, color=TEAL, ls=":", lw=1.5, label=f"Best ({best_epoch})")
ax.axhline(0.75, color=TEAL, lw=0.8, ls="--", alpha=0.4)
...
ax.plot(dates_t, qtest_pred, color=TEAL, lw=1.2, ls="--", label="CNN-LSTM", alpha=0.9)
ax.fill_between(dates_t, qtest_true, qtest_pred, alpha=0.15, color=TEAL)
ax.set_title(
...
_style(ax)
ax.scatter(qtest_true, qtest_pred, alpha=0.4, s=8, color=TEAL, edgecolors="none")
...
ax.hist(res, bins=50, color=TEAL, alpha=0.75, density=True)
...
model_bars = [
    ("LSTM", [lstm[m] for m in met_names], "#4a6a82"),
    ("Transformer", [transf[m] for m in met_names], "#00d4ff"),
    ("CNN-LSTM", [m_test[m] for m in met_names], TEAL),
]
x = np.arange(len(met_names))
w = 0.25
for i, (name, vals, c) in enumerate(model_bars):
    ax.bar(x + i * w, vals, w, label=name, color=c, alpha=0.8)
ax.axhline(0.75, color=ORG, lw=1, ls=":", alpha=0.7)
ax.set_xticks(x + w)
ax.set_xticklabels(met_names, color=TEXT)
ax.set_ylabel("Score", color=TEXT)
ax.set_title("Model Comparison", color="#e8f4f8", fontsize=11)
ax.legend(facecolor=PANEL, edgecolor=SPINE, labelcolor=TEXT, fontsize=7)
ax.set_ylim(-0.1, 1.1)

fig.suptitle(
    "CNN-LSTM — Nahr Ibrahim Rainfall–Runoff Model",
    color="#e8f4f8",
    fontsize=14,
    y=0.98,
...
plt.savefig(
    FIG_DIR / "cnn_lstm_results.png", dpi=150, bbox_inches="tight", facecolor=BG
)
plt.show()
