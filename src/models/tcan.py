...
# TCN backbone extracts local temporal patterns via dilated causal convolutions.
# Multi-head self-attention then attends globally across all 30 timesteps,
# weighting which past days are most relevant for today's discharge prediction.
# This combines TCN's translation-invariant local feature extraction with the
# Transformer's global context — particularly useful for karstic hydrology
# where both recent precipitation and longer antecedent snowmelt matter.
def build_tcan(
    input_shape, n_filters=64, kernel_size=3, n_stacks=4, n_heads=4, dropout=0.2
):
    ...
    # Attends across the 30 timesteps produced by the TCN backbone.
    # Each head can learn to focus on different temporal patterns —
    # e.g. one head for recent precipitation, another for snowmelt lag.
    attn = tf.keras.layers.MultiHeadAttention(
        num_heads=n_heads,
        key_dim=n_filters // n_heads,
        dropout=dropout,
    )(x, x)
    ...
    return tf.keras.Model(inputs, output, name="TCAN")


# ── Build and train ────────────────────────────────────────────────────────────
print("Building TCAN ...")
model = build_tcan(input_shape=(X_train.shape[1], X_train.shape[2]))
model.summary()
...
pd.DataFrame([{"model": "TCAN", **metrics}]).to_csv(
    MET_DIR / "tcan_metrics.csv", index=False
)

# ── Save model ─────────────────────────────────────────────────────────────────
model.save(MODEL_DIR / "tcan_final.keras")
print(f"\n  Saved → models/trained/tcan_final.keras")
...
ax1.scatter(y_obs, y_pred, alpha=0.3, s=6, color="#a855f7")
...
ax2.plot(
    dates, y_pred, color="#a855f7", linewidth=1.2, alpha=0.85, label="TCAN predicted"
)
...
plt.savefig(
    FIG_DIR / "tcan_results.png", dpi=150, bbox_inches="tight", facecolor="#080f1a"
)
plt.show()
print(f"  Figure saved → results/figures/tcan_results.png")
print(f"\n{'=' * 55}")
print(f"  TCAN complete  NSE={metrics['NSE']}  KGE={metrics['KGE']}")
print(f"{'=' * 55}")
