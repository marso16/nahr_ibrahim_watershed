...
from mlp_model import WatershedMLP
...
def main():
    cfg = get_config()
    set_seed(cfg.seed)
    tag = cfg.run_tag if cfg.run_tag else f"mlp_seed{cfg.seed}"
    ...
    print(f"  Flattened input: {flat_dim} dims")
    ...
    # ── Model ────────────────────────────────────────────────────────────────
    hidden_dims = tuple(int(x) for x in cfg.hidden_dims.split(","))
    model = WatershedMLP(
        input_dim=input_dim,
        seq_len=seq_len,
    ...
    print(f"\nModel: WatershedMLP {hidden_dims} | Parameters: {n_params:,}")
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        if val_loss < best_val:
            ...
            torch.save(
                ...
                MODEL_DIR / "checkpoints" / f"mlp_best_{tag}.pt",
            )
        else:
            patience_counter += 1
    ...
    ckpt = torch.load(
        MODEL_DIR / "checkpoints" / f"mlp_best_{tag}.pt", weights_only=False
    )
    model.load_state_dict(ckpt["model_state_dict"])
    model.eval()
    torch.save(model.state_dict(), MODEL_DIR / "trained" / f"mlp_final_{tag}.pt")
    pd.DataFrame(history).to_csv(
        MODEL_DIR / "configs" / f"mlp_training_log_{tag}.csv", index=False
    )
    ...
    # Save metrics (same schema as lstm_metrics_*.csv so extract_all.py picks it up)
    pd.DataFrame(
        [
            {
                "split": "Test",
    ...
    ).to_csv(MET_DIR / f"mlp_metrics_{tag}.csv", index=False)
    ...
    ).to_csv(PRED_DIR / f"mlp_predictions_test_{tag}.csv", index=False)

    print(f"\nMetrics    → results/metrics/mlp_metrics_{tag}.csv")
    print(f"Predictions → results/predictions/mlp_predictions_test_{tag}.csv")
    ...
    ax.set_title(
        f"MLP {tag} — NSE={nse_val:.3f} | KGE={kge_val:.3f} | MAE={mae_real:.2f} m³/s"
    )
    ax.set_ylabel("Discharge (m³/s)")
    ax.legend(loc="upper right")
    ax.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(FIG_DIR / f"mlp_results_{tag}.png", dpi=130, bbox_inches="tight")
    plt.close()
