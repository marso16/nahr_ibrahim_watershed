...
# ── Copy scaler to suffixed name for model scripts ────────────────────────────
import shutil

scaler_src = SPLIT_DIR / "scaler_params.csv"
scaler_dst = SPLIT_DIR / f"scaler_params{suffix}.csv"
if scaler_src.exists():
    shutil.copy(scaler_src, scaler_dst)

# ── Write window_meta.json with log-transform metadata ───────────────────────
import json

scaler_df = pd.read_csv(scaler_src, index_col=0)
log_transform = False
log_eps = 0.0
...
with open(SEQ_DIR / f"window_meta{suffix}.json", "w") as f:
