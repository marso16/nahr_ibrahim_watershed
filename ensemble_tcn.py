...
for horizon in horizons:
    ...
    print(f"\n{'='*70}")
    print(f"HORIZON {horizon} DAY ENSEMBLE")
    print(f"{'='*70}")
    ...
    for seed in seeds:
        ...
        tag = f"tcn_h{horizon}_s{seed}"

        file = PRED_DIR / f"tcn_predictions_test_{tag}.csv"
    ...
    obs = pd.read_csv(
        PRED_DIR / f"tcn_predictions_test_tcn_h{horizon}_s{seeds[0]}.csv",
        parse_dates=["date"],
    )
    ...
    final.to_csv(PRED_DIR / f"tcn_ensemble_h{horizon}.csv")
    ...
    pd.DataFrame(
        ...
        MET_DIR / f"tcn_ensemble_metrics_h{horizon}.csv",
        index=False,
    )

    print(f"Saved: tcn_ensemble_h{horizon}.csv")
