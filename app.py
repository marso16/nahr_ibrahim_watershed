...
with st.sidebar:
    ...
    page = st.radio("Navigation", [
        ...
        "✂️ Data Splits", "🤖 Model Results", "🌍 Climate Scenarios",
        "🔬 Variable Inspector", "📋 Pipeline Summary"
...
if page == "🌊 Overview":
    ...
    st.markdown("<div class='section-header'>Key Results — Test Period 2021–2025</div>", unsafe_allow_html=True)
    ...
    for i, (title, desc) in enumerate([
        ("Data collection & validation",  "CHIRPS precipitation · MERRA-2 temperature · GLDAS SWE/soil moisture · MODIS snow · GloFAS discharge"),
        ("Feature engineering",           "16 hydrological features: precip indices · temperature · cryosphere · soil moisture + anomaly · Hamon PET"),
        ("Model development",             "5 architectures: LSTM · CNN-LSTM · Transformer (pure AI) + PI-LSTM · PI-Transformer (physics-informed hybrid)"),
        ("Performance evaluation",        "7 metrics: NSE · KGE · RMSE · MAE · PBIAS · Peak Bias · Log-NSE on chronological test set 2021–2025"),
        ("Climate scenario projection",   "NEX-GDDP-CMIP6 MPI-ESM1-2-HR · SSP2-4.5 & SSP5-8.5 · 2015–2100 · Mann-Kendall trend analysis"),
    ], 1):
        st.markdown(f"""
    ...
    with col1:
        for icon, name, meta in [
            ("🏔️","Location","Mount Lebanon · Byblos District · Keserwan-Jbeil Governorate"),
            ("📐","Area & Elevation","330 km² · 2–2,684 m asl · Mean 1,571 m · Slope 0.1°–62.7°"),
            ("💧","Karstic Springs","Afqa 34.068°N 35.894°E ~1,200 m · Roueiss 34.109°N 35.908°E ~1,265 m"),
        ]:
            st.markdown(f"<div class='dataset-card'><div class='dataset-icon'>{icon}</div><div><div class='dataset-name'>{name}</div><div class='dataset-meta'>{meta}</div></div></div>", unsafe_allow_html=True)
    with col2:
        for icon, name, meta in [
            ("🌡️","Climate","Mediterranean · 900–1,400 mm/yr precip · 25–35% as snow"),
            ("🌊","River Outlet","34.062°N, 35.642°E · Mediterranean Sea · ~23 km river length"),
            ("📡","Discharge Target","GloFAS ERA5 v4.0 · 0.05° · Q range 0.227–7.312 m³/s"),
        ]:
            st.markdown(f"<div class='dataset-card'><div class='dataset-icon'>{icon}</div><div><div class='dataset-name'>{name}</div><div class='dataset-meta'>{meta}</div></div></div>", unsafe_allow_html=True)
...
# =============================================================================
    ...
    st.markdown("<div class='hero-title' style='font-family:DM Serif Display,serif;font-size:2rem;margin-bottom:.5rem;'>Data Sources</div>", unsafe_allow_html=True)
    st.markdown("<div style='font-family:DM Mono,monospace;font-size:.75rem;color:#4a6a82;margin-bottom:2rem;'>REMOTE SENSING · REANALYSIS · LAND SURFACE MODELS</div>", unsafe_allow_html=True)

    for icon, name, source, res, var, status in [
        ("🌧️","CHIRPS v2.0",          "Google Earth Engine","Daily · 0.05° · 2000–2025","Precipitation (mm/day) — gauge-corrected","✅ Ready"),
        ("🌡️","MERRA-2 T2M",          "NASA Giovanni",      "Hourly→Daily · 0.5° · 2000–2025","Temperature — Tmean/Tmax/Tmin (°C)","✅ Ready"),
        ("❄️","GLDAS Noah v2.1",       "NASA Giovanni",      "3-hourly→Daily · 0.25° · 2000–2025","SWE (mm) + Soil Moisture 0–10 cm","✅ Ready"),
        ("🏔️","MODIS MOD10A1.061",     "NASA AppEEARS",      "Daily · 500m · 2000–2025","Snow Cover (%) — NDSI ≥ 40","✅ Ready"),
        ("🌊","GloFAS ERA5 v4.0",      "Copernicus CDS",     "Daily · 0.05° · 2000–2025","River Discharge (m³/s) — LISFLOOD routing","✅ Ready"),
        ("🌍","NEX-GDDP-CMIP6",        "Google Earth Engine","Daily · 0.25° · 2015–2100","MPI-ESM1-2-HR · pr · tas · tasmin · tasmax","⏳ Downloading"),
    ]:
        ok = "Ready" in status
    ...
    st.markdown("<div class='section-header'>Spatial Configuration</div>", unsafe_allow_html=True)
    st.markdown("""
    <div class='info-box'>
    ...
    st.markdown("<div class='section-header'>16 Engineered Features</div>", unsafe_allow_html=True)
    features = [
...
# =============================================================================
    ...
    st.markdown("<div class='hero-title' style='font-family:DM Serif Display,serif;font-size:2rem;margin-bottom:.5rem;'>Exploratory Analysis</div>", unsafe_allow_html=True)
    st.markdown("<div style='font-family:DM Mono,monospace;font-size:.75rem;color:#4a6a82;margin-bottom:2rem;'>2000–2025 · DAILY · 16 FEATURES</div>", unsafe_allow_html=True)
    ...
    fig = make_subplots(rows=2, cols=3,
        subplot_titles=("Precipitation (mm/day)","Temperature (°C)","Snow Cover (%)","Discharge (m³/s)","Soil Moisture (mm)","PET (mm/day)"),
        vertical_spacing=0.15, horizontal_spacing=0.08)
    fig.add_trace(go.Bar(x=months, y=monthly["precip_mm_day"], marker_color="#3b9eff", marker_opacity=0.85, name="Precip"), row=1, col=1)
    fig.add_trace(go.Scatter(x=months, y=monthly["temp_mean_c"], line=dict(color="#f4a261",width=2.5), fill="tozeroy", fillcolor="rgba(244,162,97,.08)", mode="lines+markers", marker=dict(size=7,color="#f4a261"), name="Temp"), row=1, col=2)
    fig.add_trace(go.Bar(x=months, y=monthly["snow_cover_pct"], marker_color="#a8d8ea", marker_opacity=0.85, name="Snow"), row=1, col=3)
    fig.add_trace(go.Bar(x=months, y=monthly["discharge_m3s"], marker_color="#00b4a0", marker_opacity=0.85, name="Q"), row=2, col=1)
    if "soil_moisture_mm" in monthly.columns:
        fig.add_trace(go.Scatter(x=months, y=monthly["soil_moisture_mm"], line=dict(color="#a855f7",width=2.5), fill="tozeroy", fillcolor="rgba(168,85,247,.08)", mode="lines+markers", marker=dict(size=7,color="#a855f7"), name="SM"), row=2, col=2)
    if "pet_mm_day" in monthly.columns:
        fig.add_trace(go.Bar(x=months, y=monthly["pet_mm_day"], marker_color="#e76f51", marker_opacity=0.85, name="PET"), row=2, col=3)
    fig.update_layout(height=580, showlegend=False, **PLOT_LAYOUT)
    st.plotly_chart(fig, use_container_width=True)

    st.markdown("<div class='section-header'>Time Series Explorer</div>", unsafe_allow_html=True)
    var_options = {
        "Discharge (m³/s)":"discharge_m3s","Precipitation (mm/day)":"precip_mm_day",
        "Temperature (°C)":"temp_mean_c","Snow Cover (%)":"snow_cover_pct",
        "SWE (mm)":"swe_mm","Soil Moisture (mm)":"soil_moisture_mm",
        "PET (mm/day)":"pet_mm_day","SM Anomaly (mm)":"sm_anomaly",
    }
    col1, col2 = st.columns([3,1])
    with col2:
        selected_var = st.selectbox("Variable", [k for k,v in var_options.items() if v in df.columns])
        year_range   = st.slider("Year range", 2000, 2025, (2000, 2025))
    var_col     = var_options[selected_var]
    df_f        = df[(df["date"].dt.year >= year_range[0]) & (df["date"].dt.year <= year_range[1])]
    cmap        = {"discharge_m3s":"#00b4a0","precip_mm_day":"#3b9eff","temp_mean_c":"#f4a261","snow_cover_pct":"#a8d8ea","swe_mm":"#00d4ff","soil_moisture_mm":"#a855f7","pet_mm_day":"#e76f51","sm_anomaly":"#f4a261"}
    c           = cmap.get(var_col,"#3b9eff")
    fig2 = go.Figure(go.Scatter(x=df_f["date"], y=df_f[var_col], fill="tozeroy",
        fillcolor=f"rgba({int(c[1:3],16)},{int(c[3:5],16)},{int(c[5:7],16)},.15)",
        line=dict(color=c,width=1.2), mode="lines", name=selected_var))
    fig2.update_layout(height=350, **PLOT_LAYOUT, yaxis_title=selected_var)
    with col1: st.plotly_chart(fig2, use_container_width=True)

    st.markdown("<div class='section-header'>Correlation Matrix</div>", unsafe_allow_html=True)
    corr_cols = [c for c in ["precip_mm_day","temp_mean_c","swe_mm","snow_cover_pct","swe_delta","precip_3day","soil_moisture_mm","pet_mm_day","discharge_m3s"] if c in df.columns]
    corr = df[corr_cols].corr()
    short = ["Precip","Temp","SWE","Snow%","ΔSWE","P3d","SM","PET","Q"][:len(corr_cols)]
    fig3 = go.Figure(go.Heatmap(z=corr.values, x=short, y=short,
        colorscale=[[0,"#e76f51"],[.5,"#111e2e"],[1,"#3b9eff"]], zmin=-1, zmax=1,
        text=np.round(corr.values,2), texttemplate="%{text}",
        textfont=dict(size=11, family="DM Mono, monospace"), showscale=True))
...
# =============================================================================
    ...
    st.markdown("<div class='hero-title' style='font-family:DM Serif Display,serif;font-size:2rem;margin-bottom:.5rem;'>Train / Validation / Test Split</div>", unsafe_allow_html=True)
    st.markdown("<div style='font-family:DM Mono,monospace;font-size:.75rem;color:#4a6a82;margin-bottom:2rem;'>CHRONOLOGICAL · NO SHUFFLING · 30-DAY LOOKBACK · 16 FEATURES</div>", unsafe_allow_html=True)
    ...
    for name, data, color, fill in [
        ...
        if data is not None:
            fig.add_trace(go.Scatter(x=data["date"], y=data["discharge_m3s"], fill="tozeroy", fillcolor=fill, line=dict(color=color,width=1), name=name))
    ...
    fig2 = go.Figure()
    for name, data, lc, fc in [("Train",train,"#3b9eff","rgba(59,158,255,.2)"),("Validation",val,"#f4a261","rgba(244,162,97,.2)"),("Test",test,"#e76f51","rgba(231,111,81,.2)")]:
        if data is not None:
            fig2.add_trace(go.Violin(y=data["discharge_m3s"], name=name, fillcolor=fc, line_color=lc, box_visible=True, meanline_visible=True, points=False))
    ...
    for name, data in [("Train",train),("Validation",val),("Test",test)]:
        if data is not None:
            row = {"Split": name}
            for col in ["precip_mm_day","temp_mean_c","swe_mm","snow_cover_pct","discharge_m3s"]:
                if col in data.columns: row[col] = f"{data[col].mean():.3f}"
            stats_data.append(row)
    sdf = pd.DataFrame(stats_data)
    sdf.columns = ["Split","Precip (mm/d)","Temp (°C)","SWE (mm)","Snow (%)","Q (m³/s)"]
...
# =============================================================================
    ...
    st.markdown("<div class='hero-title' style='font-family:DM Serif Display,serif;font-size:2rem;margin-bottom:.5rem;'>Model Benchmark Results</div>", unsafe_allow_html=True)
    st.markdown("<div style='font-family:DM Mono,monospace;font-size:.75rem;color:#4a6a82;margin-bottom:2rem;'>TEST PERIOD 2021–2025 · 16 FEATURES · CHIRPS PRECIPITATION</div>", unsafe_allow_html=True)

    st.markdown("<div class='section-header'>Performance Metrics — Test Set</div>", unsafe_allow_html=True)
    st.markdown("""
    ...
    st.markdown("<div class='section-header'>Visual Metric Comparison</div>", unsafe_allow_html=True)
    ...
    fig = go.Figure(go.Bar(x=models_list, y=vals, marker_color=colors_m, marker_opacity=0.85,
        text=[f"{v:.3f}" for v in vals], textposition="outside",
        textfont=dict(family="DM Mono, monospace", size=11, color="#e8f4f8")))
    if metric in ["NSE","KGE"]:
        fig.add_hline(y=0.75, line_dash="dot", line_color="#00b4a0", annotation_text="Good (0.75)")
    fig.update_layout(height=380, **PLOT_LAYOUT, yaxis_title=metric)
    st.plotly_chart(fig, use_container_width=True)

    st.markdown("<div class='section-header'>Feature Impact — 12 vs 16 Features</div>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    with col1:
        fig2 = go.Figure()
        fig2.add_trace(go.Bar(name="12 features", x=["LSTM","Transformer","PI-Transformer"], y=[0.518,0.603,0.528], marker_color="#4a6a82", marker_opacity=0.8))
        fig2.add_trace(go.Bar(name="16 features", x=["LSTM","Transformer","PI-Transformer"], y=[0.515,0.680,0.586], marker_color="#3b9eff", marker_opacity=0.8))
        fig2.update_layout(height=320, barmode="group", title_text="NSE", **PLOT_LAYOUT)
        st.plotly_chart(fig2, use_container_width=True)
    with col2:
        fig3 = go.Figure()
        fig3.add_trace(go.Bar(name="12 features", x=["LSTM","Transformer","PI-Transformer"], y=[0.503,0.632,0.574], marker_color="#4a6a82", marker_opacity=0.8))
        fig3.add_trace(go.Bar(name="16 features", x=["LSTM","Transformer","PI-Transformer"], y=[0.423,0.671,0.730], marker_color="#a855f7", marker_opacity=0.8))
...
# =============================================================================
    ...
    st.markdown("<div class='hero-title' style='font-family:DM Serif Display,serif;font-size:2rem;margin-bottom:.5rem;'>Climate Scenario Projections</div>", unsafe_allow_html=True)
    st.markdown("<div style='font-family:DM Mono,monospace;font-size:.75rem;color:#4a6a82;margin-bottom:2rem;'>MPI-ESM1-2-HR · NEX-GDDP-CMIP6 · 2015–2100</div>", unsafe_allow_html=True)
    ...
    if not scenarios:
        ...
        st.markdown("<div class='section-header'>Expected Results (Previous Run)</div>", unsafe_allow_html=True)
        st.markdown("""
        <table class='results-table'>
        ...
        st.markdown("<div class='section-header'>Annual Discharge Projections</div>", unsafe_allow_html=True)
        fig = make_subplots(rows=2, cols=1,
            subplot_titles=["SSP2-4.5 — Moderate Emissions","SSP5-8.5 — High Emissions"],
            vertical_spacing=0.12)
        model_colors = {"Q_LSTM":"#3b9eff","Q_CNN-LSTM":"#00b4a0","Q_Transformer":"#00d4ff","Q_PI-LSTM":"#f4a261","Q_PI-Transformer":"#e76f51","Q_ensemble_all":"#ffffff"}
        for row_i, (scen, df_s) in enumerate(scenarios.items(), 1):
            df_s = df_s.copy(); df_s["year"] = df_s["date"].dt.year
            ...
            for col, color in model_colors.items():
                ...
                fig.add_trace(go.Scatter(x=annual.index, y=sm,
                    line=dict(color=color, width=2.5 if col=="Q_ensemble_all" else 1.2),
                    name=col.replace("Q_",""), showlegend=(row_i==1)), row=row_i, col=1)
            fig.add_vline(x=2025, line_dash="dot", line_color="#4a6a82", row=row_i, col=1)
...
# =============================================================================
    ...
    st.markdown("<div class='hero-title' style='font-family:DM Serif Display,serif;font-size:2rem;margin-bottom:.5rem;'>Variable Inspector</div>", unsafe_allow_html=True)

    var_map = {k:v for k,v in {
        "Discharge (m³/s)":"discharge_m3s","Precipitation (mm/d)":"precip_mm_day",
        "Temperature (°C)":"temp_mean_c","Temp max (°C)":"temp_max_c","Temp min (°C)":"temp_min_c",
        "SWE (mm)":"swe_mm","Snow Cover (%)":"snow_cover_pct","SWE Delta":"swe_delta",
        "Precip 3-day":"precip_3day","Precip 7-day":"precip_7day",
        "Soil Moisture (mm)":"soil_moisture_mm","SM 7-day Mean":"sm_7day_mean",
        "SM Anomaly":"sm_anomaly","PET (mm/day)":"pet_mm_day",
    }.items() if v in df.columns}
    ...
    for container, label, val in [(c1,"Mean",f"{series.mean():.3f}"),(c2,"Std",f"{series.std():.3f}"),(c3,"Min",f"{series.min():.3f}"),(c4,"Max",f"{series.max():.3f}"),(c5,"Median",f"{series.median():.3f}")]:
        container.markdown(f"<div class='metric-card' style='margin-bottom:1rem;'><div class='metric-label'>{label}</div><div style='font-family:DM Serif Display,serif;font-size:1.5rem;color:#00d4ff;'>{val}</div></div>", unsafe_allow_html=True)

    fig = make_subplots(rows=1, cols=2, column_widths=[.7,.3], subplot_titles=("Time Series","Distribution"))
    fig.add_trace(go.Scatter(x=df["date"], y=df[col], line=dict(color="#3b9eff",width=.8), fill="tozeroy", fillcolor="rgba(59,158,255,.1)", name=selected), row=1, col=1)
    fig.add_trace(go.Histogram(y=series, nbinsy=60, marker_color="#3b9eff", marker_opacity=.7, name="Dist"), row=1, col=2)
    ...
    fig3 = go.Figure()
    fig3.add_trace(go.Bar(x=annual.index, y=annual.values, marker_color="#3b9eff", marker_opacity=.7, name="Annual mean"))
    fig3.add_trace(go.Scatter(x=annual.index, y=np.poly1d(z)(annual.index), line=dict(color="#e76f51",width=2,dash="dash"), name=f"Trend ({z[0]:+.4f}/yr)"))
...
# =============================================================================
    ...
    st.markdown("<div class='hero-title' style='font-family:DM Serif Display,serif;font-size:2rem;margin-bottom:.5rem;'>Pipeline Summary</div>", unsafe_allow_html=True)
    st.markdown("<div style='font-family:DM Mono,monospace;font-size:.75rem;color:#4a6a82;margin-bottom:2rem;'>DATA ENGINEERING · MODELLING · STATUS</div>", unsafe_allow_html=True)

    for icon, title, desc in [
        ("✅","Data Collection","CHIRPS v2.0 (GEE, gauge-corrected) · MERRA-2 temperature · GLDAS Noah SWE + soil moisture 0–10 cm · MODIS MOD10A1 snow cover · GloFAS ERA5 discharge · CMIP6 MPI-ESM1-2-HR via GEE (downloading)."),
        ("✅","Input Data Validation","MERRA-2 vs Meteostat WMO 40100: temp r=0.975 daily / r=0.994 monthly / bias=−2.4°C. CHIRPS vs airport: r=0.513 daily / r=0.653 monthly / PBIAS=−6.2%."),
        ("✅","Preprocessing","Gap filling (linear interpolation ≤3 days). Min-max normalisation on training set only. 30-day lookback windowing. Shape: (samples, 30, 16)."),
        ("✅","MODIS Processing","27,249 GeoTIFFs clipped to watershed polygon via rasterio.mask. NDSI ≥ 40. 19,984 duplicates removed → 7,265 unique days."),
        ("✅","GloFAS Extraction","312 yearly GRIB files. dis24 at outlet 34.062°N, 35.642°E. Q range: 0.227–7.312 m³/s. 9,497 daily rows."),
        ("✅","Feature Engineering","16 features: precip (1/3/7-day) · temp (mean/min/max/range) · SWE + delta · snow cover · month sin/cos · soil moisture + 7-day mean + anomaly · Hamon PET."),
        ("✅","Train/Val/Test Split","Train 2000–2017 (~6,515) · Val 2018–2020 (1,066) · Test 2021–2025 (1,796). Chronological — no shuffling."),
        ("✅","Model Training","LSTM (125K) · CNN-LSTM (143K) · Transformer (108K) · PI-LSTM · PI-Transformer. PI loss: L = MSE + 0.05·(P−ET−Q−ΔS)²."),
        ("✅","Evaluation","Best NSE=0.680 (Transformer) · Best KGE=0.730 (PI-Transformer) · Best Peak Bias=−29.7% (PI-Transformer)."),
        ("⏳","Climate Scenarios","CMIP6 download via GEE in progress — 688 files (pr/tas/tasmin/tasmax · ssp245+ssp585 · 2015–2100). Run climate_scenarios.py after."),
        ("🔜","Thesis Writing","Chapter 4 Results · Chapter 5 Discussion · Chapter 6 Conclusion. Chapters 1–3 complete."),
    ]:
    ...
    for folder, contents in [
        ...
        ("data/raw/cmip6/",     "ssp245/{pr,tas,tasmin,tasmax}/{year}.csv · ssp585/... (⏳ downloading)"),
        ("data/master/",        "nahr_ibrahim_master_full.csv (9,497 rows × 17 cols)"),
        ("data/splits/",        "train_raw.csv · val_raw.csv · test_raw.csv · scaler_params.csv"),
        ("data/sequences/",     "X_train.npy (6515,30,16) · X_val.npy · X_test.npy · y_*.npy"),
        ("models/trained/",     "lstm_final.keras · cnn_lstm_final.keras · transformer_final.keras · pi_lstm_final.keras · pi_transformer_final.keras"),
        ("results/metrics/",    "pi_models_metrics.csv · climate_scenario_trends_full.csv"),
        ("results/figures/",    "lstm_results.png · cnn_lstm_results.png · transformer_results.png · pi_models_results.png · scenario_*.png"),
        ("qgis/",               "nahr_ibrahim_watershed_utm37n.shp · dem_nahr_ibrahim_clipped.tif · slope_nahr_ibrahim.tif · hillshade_nahr_ibrahim.tif · key_locations.csv"),
    ]:
        st.markdown(f"""
